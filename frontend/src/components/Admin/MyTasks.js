...
export default function MyTasks() {
  ...
  const dueSoon = useMemo(() => {
    ...
    return tasks.filter((task) => !task.completed && task.due_date).reduce((counts, task) => {
      const due = new Date(`${String(task.due_date).slice(0, 10)}T00:00:00`);
      if (due < today) counts.overdue += 1;
      else if (due <= tomorrow) counts.upcoming += 1;
      return counts;
  ...
  return <main className="tasks-workspace">
    ...
    {(dueSoon.overdue > 0 || dueSoon.upcoming > 0) && <div className="task-due-alert" role="status"><FaCalendarAlt />{' '}
      {dueSoon.overdue > 0 && `${dueSoon.overdue} overdue task${dueSoon.overdue === 1 ? '' : 's'}`}
      {dueSoon.overdue > 0 && dueSoon.upcoming > 0 && ' · '}
      {dueSoon.upcoming > 0 && `${dueSoon.upcoming} due today or tomorrow`}
    </div>}
