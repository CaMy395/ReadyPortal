...
const BartendingClassSection = ({ bartendingClasses }) => {
  ...
  const visibleForms = bartendingClasses.filter(form => showHidden || !hiddenIds.includes(form.id));

  return (
    <div className="table-scroll-container">
      <h2>Bartending Class Forms</h2>
      ...
      {visibleForms.length > 0 ? (
        ...
        <p>No Bartending Class forms submitted yet.</p>
      )}
    </div>
  );
};

export default BartendingClassSection;
