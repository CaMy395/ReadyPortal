...
const normalizeEditQuote = (data) => ({
  ...
  quote_date: toISODate(data.quote_date || data.date),
...
const QuotesPreviewPage = () => {
  ...
  const amountPaid = useMemo(() => {
    if (!quote) return 0;
    return parseFloat(quote.amount_paid || 0) || 0;
  }, [quote]);

  const balance = useMemo(() => {
    if (!quote) return '0.00';

    const dbBalance = quote.balance_due;
    if (dbBalance !== null && dbBalance !== undefined) {
      return parseFloat(dbBalance || 0).toFixed(2);
    }

    return Math.max(calculatedTotal - amountPaid, 0).toFixed(2);
  }, [quote, calculatedTotal, amountPaid]);

  const isPaid = parseFloat(balance) <= 0;

  const editingBalance = useMemo(() => {
    const paid = parseFloat(editQuote?.amount_paid || 0) || 0;
    return Math.max(editingCalculatedTotal - paid, 0).toFixed(2);
  }, [editingCalculatedTotal, editQuote]);
  ...
  const handleItemChange = (idx, field, value) => {
    setEditQuote((q) => {
      ...
      items[idx] = {
        ...items[idx],
        [field]: field === 'quantity' ? (value === '' ? '' : parseInt(value || 0, 10)) : value
      };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const freshRes = await fetch(`${apiUrl}/api/quotes/${id}`);
      const freshData = await freshRes.json();

      if (!freshRes.ok) {
        throw new Error(freshData?.error || 'Saved, but failed to reload quote');
      }

      setQuote(freshData);
      setEditQuote(normalizeEditQuote(freshData));
      setIsEditing(false);
    } catch (e) {
      console.error(e);
  ...
  const cancelEdit = () => {
    if (quote) {
      setEditQuote(normalizeEditQuote(quote));
    }
  ...
  if (!quote || !editQuote) {
    return <p style={{ padding: 20 }}>Loading quote...</p>;
  }
      ...
      {isEditing ? (
        <>
          ...
          <Row label="Amount Paid">
            <span style={{ color: amountPaid > 0 ? 'green' : '#333', fontWeight: 600 }}>
              ${amountPaid.toFixed(2)}
            </span>
          </Row>

          <Row label="Balance (view)">
            <div style={{ color: parseFloat(editingBalance) > 0 ? 'red' : 'green', fontWeight: 600 }}>
              ${editingBalance}
            </div>
          </Row>
        </>
        ...
        <>
          ...
          <Row label="Balance">
            <span style={{ color: parseFloat(balance) > 0 ? 'red' : 'green', fontWeight: 600 }}>
              ${balance}
            </span>
          </Row>
